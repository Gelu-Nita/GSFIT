  ...
  if keyword_set(getlib) then begin
    case !version.os_family of
      'Windows':begin 
              ...
              unix_dir = file_search(getenv('gsfitpath'), 'unix', /mark)
              libpath = gsfit_libpath(unix_dir)
              if libpath EQ !NULL THEN BEGIN
                message, 'Using user-local compiled library from gsfit_binaries.', /info
              ENDIF ELSE BEGIN
...
pro gsfit_refit,state,start=start
 ...
 desc=[$
                   ...
                   '1, BASE,, Row, FRAME', $
                   '2, BUTTON, Recompute solution for selected pixel|Recompute solutions inside the selected ROI and time range|Recompute all solutions in the selected time range, Exclusive, Column,SET_VALUE='+(roi_count gt 0?'1':'0')+',LABEL_LEFT='', TAG=redo', $
                   '1, BASE,, ROW,Frame', $
