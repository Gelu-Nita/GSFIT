...
;    and convert them to a map cube either in Tb or sfu units
; CATEGORY:
;    EOVSA
; CALLING SEQUENCE:
;      maps=eovsa_fits2cube(files,no_iv2rl=no_iv2rl,flux_threshold=flux_threshold,rms_mask=rms_mask,sfu=sfu,no_rms=no_rms)
;
; INPUTS:
...
function eovsa_fits2cube,files,no_iv2rl=no_iv2rl,flux_threshold=flux_threshold,rms_mask=rms_mask,sfu=sfu,no_rms=no_rms
 ...
 if ~keyword_set(no_rms) then begin
   if ~has_rms or n_elements(rms_mask) eq n_elements(maps[0].data) then begin
     default,rms_mask,(total(total(reform(maps.data,nx,ny,nfreq,npol,ntime),/nan,3),3) lt ntime*flux_threshold)
     idx=where(rms_mask)
     for i=0,nfreq-1 do begin
